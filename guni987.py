...
SENDER_EMAIL = "mentor4566@gmail.com"
GMAIL_APP_PASSWORD = "fujgdplfzngzjysb"
RECIPIENT_EMAILS = {
    "mentor": "mentor4566@gmail.com",
...
def init_db():
    ...
    if "enrollment_private" not in cols:
        cur.execute("ALTER TABLE problems ADD COLUMN enrollment_private INTEGER DEFAULT 1")
    cur.execute("""
...
"""
    ...
    try:
        print("\n========== GMAIL NOTIFICATION DEBUG ==========")
        print("Sender Email:", SENDER_EMAIL)
        print("Recipient Email:", to_email)
        ...
        print("SUCCESS: Gmail notification sent successfully!")
        print("===============================================\n")
        return True
...
<label>Enrollment Number Privacy</label>
<div class="radio">
<label><input type="radio" name="enrollment_private" value="yes" checked> 🔒 Keep my enrollment number secret</label>
<label><input type="radio" name="enrollment_private" value="no"> 🔓 Reveal my enrollment number</label>
</div>
<div class="info">🔐 If you keep it secret, Mentor/HOD/Principal will see <b>Secret Enrollment</b> instead of your actual enrollment number. Your enrollment number is still used internally to check your status.</div>
...
def submit():
    ...
    enrollment_private = request.form.get("enrollment_private", "yes").strip()
    ...
    if enrollment_private not in {"yes", "no"}:
        return "Please choose a valid enrollment privacy option.", 400

    enrollment_private_value = 1 if enrollment_private == "yes" else 0
    ...
    cur.execute("""INSERT INTO problems
        (name,enrollment,description,recipient,is_anonymous,enrollment_private)
        VALUES (?,?,?,?,?,?)""",
        (display_name,enrollment,description,recipient,is_anonymous,enrollment_private_value))
    ...
    conn.commit()
    conn.close()
    enrollment_for_email = "Secret Enrollment" if enrollment_private_value else enrollment
    email_sent = send_email(display_name, enrollment_for_email, description, recipient, saved)

    return render_template_string(SUCCESS_PAGE,
        enrollment=enrollment,
        recipient=ROLES[recipient]["name"],
        enrollment_private=enrollment_private_value,
...
{% if email_sent %}
<p style="color:#16a34a;font-weight:bold">📧 Gmail Notification Sent Successfully!</p>
{% else %}
<p style="color:#dc2626;font-weight:bold">⚠️ Complaint saved, but Gmail notification failed. Check CMD terminal for the exact error.</p>
{% endif %}
<p><b>Sent to:</b> {{recipient}}</p>
<p><b>Enrollment Privacy:</b> {% if enrollment_private %}🔒 Secret{% else %}🔓 Revealed{% endif %}</p>
<p><b>Problem ID:</b> #{{problem_id}}</p>
...
<p><b>Enrollment:</b> {% if p.enrollment_private %}🔒 Secret Enrollment{% else %}{{p.enrollment}}{% endif %}</p>
...
{% if p.solution %}<div class="solution"><h3>✅ Solution</h3><p>{{p.solution}}</p>
{% if item.solution_attachments %}<div class="media"><h3>📷 Solution Photos / Videos</h3>
{% for f in item.solution_attachments %}
{% if f.file_type=="image" %}<img src="{{url_for('uploaded_file',filename=f.filename)}}" alt="Solution image">
{% else %}<video controls><source src="{{url_for('uploaded_file',filename=f.filename)}}">Your browser does not support video playback.</video>{% endif %}
<p>📎 {{f.original_name}}</p>{% endfor %}</div>{% endif %}
</div>
...
def status():
    ...
    for p in problems:
        ...
        cur.execute("SELECT * FROM solution_attachments WHERE problem_id=? ORDER BY id", (p["id"],))
        solution_attachments = cur.fetchall()
        data.append({
            "problem": p,
...
<style>
body{margin:0;font-family:Arial;background:#f1f5f9}header{background:#312e81;color:white;padding:25px;text-align:center}.container{max-width:1000px;margin:30px auto;padding:20px}.top{display:flex;justify-content:space-between;align-items:center;margin-bottom:25px}.logout{background:#dc2626;color:white;padding:10px 16px;border-radius:8px;text-decoration:none}.problem,.empty{background:white;padding:25px;margin-bottom:20px;border-radius:15px;box-shadow:0 5px 20px #00000012}textarea,input[type=file]{width:100%;padding:12px;border:1px solid #cbd5e1;border-radius:8px;box-sizing:border-box;margin-top:10px;background:#fff}button{background:#4f46e5;color:white;border:0;padding:12px 20px;margin-top:10px;border-radius:8px}.pending{color:#d97706;font-weight:bold}.solved{color:#16a34a;font-weight:bold}.solution{background:#ecfdf5;padding:15px;border-radius:10px}.media{margin-top:20px;background:#f8fafc;padding:15px;border-radius:12px}.media img{max-width:100%;max-height:500px;display:block;margin:15px auto;border-radius:10px}.media video{width:100%;max-height:500px;display:block;margin:15px auto;border-radius:10px}
</style></head><body>
...
<p><b>Enrollment Number:</b>
{% if p.enrollment_private %}🔒 <span style="color:#b45309;font-weight:bold">Secret Enrollment</span>
{% else %}{{p.enrollment}}{% endif %}</p>
...
{% else %}<video controls><source src="{{url_for('uploaded_file',filename=f.filename)}}">Your browser does not support video playback.</video>{% endif %}
<p>📎 {{f.original_name}}</p>{% endfor %}</div>{% endif %}
{% if p.status=="Pending" %}<form action="/{{role}}/solve/{{p.id}}" method="POST" enctype="multipart/form-data">
<label><b>Write Solution</b></label>
<textarea name="solution" rows="5" required placeholder="Write solution for the student..."></textarea>
<label><b>📷 Attach Solution Photos / 🎥 Videos</b></label>
<input type="file" name="solution_attachments" accept="image/*,video/*" multiple>
<small style="display:block;margin-top:7px;color:#64748b">You can upload photos or short videos as part of the solution. Maximum total upload size: <b>50 MB</b>.</small>
<button type="submit">✅ Send Solution</button></form>
{% else %}<div class="solution"><h3>✅ Solution Sent</h3><p>{{p.solution}}</p>
{% if item.solution_attachments %}<div class="media"><h3>📷 Solution Photos / Videos</h3>
{% for f in item.solution_attachments %}
{% if f.file_type=="image" %}<img src="{{url_for('uploaded_file',filename=f.filename)}}" alt="Solution image">
{% else %}<video controls><source src="{{url_for('uploaded_file',filename=f.filename)}}">Your browser does not support video playback.</video>{% endif %}
...
def solve(role, problem_id):
    ...
    files = []
    for f in request.files.getlist("solution_attachments"):
        if f and f.filename:
            if not allowed_file(f.filename):
                return "Invalid solution file type. Upload an image or video.", 400
            files.append(f)

    conn = sqlite3.connect(DATABASE)
    cur = conn.cursor()

    cur.execute("SELECT id FROM problems WHERE id=? AND recipient=?", (problem_id, role))
    if not cur.fetchone():
        conn.close()
        return "Problem not found or you are not allowed to solve it.", 404
    ...
    for f in files:
        ...
        f.save(os.path.join(UPLOAD_FOLDER, unique))
        typ = file_type(original)
        cur.execute("""INSERT INTO solution_attachments
            (problem_id,filename,original_name,file_type)
            VALUES (?,?,?,?)""",
            (problem_id, unique, original, typ))
